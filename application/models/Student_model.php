...
class Student_model extends CI_Model
{
	...
	public function get_student($id = null, $inactive = null)
	{
		if (!empty($id)) {
		...
		if (!empty($inactive)) {
			$this->db->where('dq_enroll.active', 0);
		}

		$this->db->select('dq_enroll.admission_number,dq_enroll.remarks,dq_enroll.old_admission_number,
		dq_enroll.admission_type,dq_enroll.fees_type,dq_enroll.class_id,dq_enroll.ac_year,
		dq_enroll.active,dq_enroll.date,dq_enroll.date_hijri,dq_students.id,dq_students.gr_number,dq_students.name,
	...
	public function detail_student($student_id = null)
	{
		...
		if (!empty($student_id))
		{
			return $this->get_student_profile($student_id);
		}
		...
		{
			$student_id = $this->get_student_id_by_admission_no($detail);
			if (!empty($student_id))
			{
				return $this->get_student_profile($student_id);
			}
		...
		if ($option == 'gr_number')
		{
			$student_id = $this->get_student_id_by_gr_number($detail);
			if (!empty($student_id))
			{
				return $this->get_student_profile($student_id);
			}
		...
		//******Search By name, father name, father cnic and area****
		if (in_array($option, ['name' , 'father_name', 'father_cnic', 'area']))
		{
			if (!empty($class_id))
			{
			...
			if ($option == 'name')
			{
				$this->db->like('name', $detail, 'both');
			}
			elseif ($option == 'father_name')
			{
				$this->db->like('father_name', $detail, 'both');
			}
			elseif ($option == 'father_cnic')
			{
				$this->db->group_start();
				$this->db->like('father_cnic', $detail);
				$this->db->or_like("REPLACE(`father_cnic`,'-','')", $detail);
				$this->db->group_end();
			}
			elseif ($option == 'area')
			{
				$this->db->where('area', $this->input->post('area_id'));
			}
		...
		redirect("Search/index/$option/");
	}

	public function get_student_id_by_admission_no($admission_no)
	{
		$this->db->select('student_id');
	...
	public function get_student_id_by_gr_number($gr_number)
	{
		$this->db->select('id');
		$this->db->where('gr_number', $gr_number);
		$this->db->where('is_delete', 0);
		$student = $this->db->get('dq_students')->row();
		return !empty($student) ? $student->id : null;
	}

	public function get_student_profile($student_id)
	{
		$this->db->select('dq_students.id,dq_students.gr_number,dq_students.name,dq_students.father_name,
	...
	{
		$this->db->select_max('gr_number');
		$data = $this->db->get('dq_students')->row();

		return $data->gr_number ? $data->gr_number + 1 : RESERVED_GR_NUMBER;
	}
}
