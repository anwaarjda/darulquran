...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', users.email, users.id, 
		users.password, users.active, users.last_login,groups.name,users.first_name')
						  ->where($this->identity_column, $identity)
						  ->limit(1)
	...
	public function set_session($user)
	{
		...
		$active_year = $this->get_active_year();
		...
		$session_data = [
			...
			'group'				   => $user->name,
		    'last_check'           => time(),
			'ac_year'			   => $active_year->ac_year,
			'active'			   => $active_year->status,
			'hijri_year'		   => $active_year->hijri_year,
		];
	...
	public function get_active_year()
	{
		...
		return $this->db->get('dq_years')->row();
	}
}
